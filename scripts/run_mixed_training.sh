#!/bin/bash
# Mixed SDSS + DECaLS Training Pipeline
# Runs head training followed by full fine-tuning

set -e  # Exit on error

echo "=== Galaxy Sommelier Mixed SDSS+DECaLS Training Pipeline ==="
echo "Date: $(date)"
...
echo "Configuration: configs/mixed_head_training_config.yaml"
echo "Training for 5 epochs with frozen DINOv2 backbone"

# Run head training
python scripts/train_baseline.py \
    --config configs/mixed_head_training_config.yaml \
    --wandb \
    --no-resume

# Check if head training completed successfully
if [ ! -f "/pscratch/sd/s/sihany/galaxy-sommelier-data/models/mixed/best_model.pt" ]; then
...
echo "Configuration: configs/mixed_full_finetuning_config.yaml"
...
python scripts/train_baseline.py \
    --config configs/mixed_full_finetuning_config.yaml \
    --wandb \
    --no-resume

echo "=== Training Pipeline Completed ==="
echo "Final model saved at: /pscratch/sd/s/sihany/galaxy-sommelier-data/models/mixed/best_model.pt"
echo "Results saved at: /pscratch/sd/s/sihany/galaxy-sommelier-data/results/mixed/"

# Show final model info
if [ -f "/pscratch/sd/s/sihany/galaxy-sommelier-data/models/mixed/best_model.pt" ]; then
    echo "Final model file size: $(du -h /pscratch/sd/s/sihany/galaxy-sommelier-data/models/mixed/best_model.pt)"
